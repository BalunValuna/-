/**
 * Seeded 2D/3D simplex noise (Gustavson) with fbm and ridged helpers.
 */
...
export class Noise {
  constructor(seed = 1) {
    ...
    for (let i = 0; i < 512; i++) this.permMod12[i] = perm[i] % 12;
  }
  ...
  noise3(x, y, z) {
    ...
    const corner = (tx, ty, tz, gi) => {
      ...
      const g = gi * 3;
      tt *= tt;
      return tt * tt * (GRAD3[g] * tx + GRAD3[g + 1] * ty + GRAD3[g + 2] * tz);
    };
    const n =
      corner(x0, y0, z0, permMod12[ii + perm[jj + perm[kk]]]) +
      corner(x1, y1, z1, permMod12[ii + i1 + perm[jj + j1 + perm[kk + k1]]]) +
      corner(x2, y2, z2, permMod12[ii + i2 + perm[jj + j2 + perm[kk + k2]]]) +
      corner(x3, y3, z3, permMod12[ii + 1 + perm[jj + 1 + perm[kk + 1]]]);
    return 32 * n;
  }
